...
| 役割                         | 技術                                                  | 備考                                                                                                          |
| ---------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| UIフレームワーク             | **Vue 3.6** (Composition API + **Vapor Mode**)        | VDOMレス高速レンダリング。Options APIは無効化                                                                 |
| UIコンポーネントライブラリ   | **Park UI** (`@ark-ui/vue` + `@park-ui/panda-preset`) | Panda CSS ネイティブの pre-styled コンポーネント。CLI でプロジェクトにコピーして使う                          |
| スタイリング / design tokens | **Panda CSS**                                         | ゼロランタイム。`sva`/`cva`/`css` でカスタムUI定義。`styled-system/` に出力。Park UI のレシピも同じ設定で管理 |
| 状態管理                     | Pinia                                                 |                                                                                                               |
